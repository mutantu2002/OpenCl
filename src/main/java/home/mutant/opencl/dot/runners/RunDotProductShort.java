 	...
 	public static void main(String[] args) throws Exception {
		...
		for (int i=0;i<10;i++){
			ObtainFiltersDotShort  of = new ObtainFiltersDotShort(imagesByType.get(i)).
					setDimFilterX(26).setDimFilterY(26).setNoClusters(noClusters).setNoIterations(noIterations).build();
			of.cluster();
		...
		MaxIndexImage mii = new MaxIndexImage(ti.getTransformedImages(),MnistDatabase.testLabels.subList(0, 256*39),noClusters*9);
